...
Approve the AI-suggested playbook and save it as a real playbook. Optionally edit the steps before approving.

```bash
curl -X POST "http://127.0.0.1:8000/api/threats/1/approve-playbook" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Ransomware Response",
    "description": "AI-suggested playbook for ransomware containment",
    "steps": [
...
If `steps` is omitted, the original AI suggestion is used as-is.

### DELETE /api/threats/{threat_id}/dismiss-playbook

Dismiss the AI-suggested playbook.

```bash
curl -X DELETE "http://127.0.0.1:8000/api/threats/1/dismiss-playbook"
```
