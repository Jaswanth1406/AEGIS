...
def dashboard_stats(_: User = Depends(get_current_user), db: Session = Depends(get_db)) -> DashboardStatsResponse:
...
def attacks_global(_: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[AttackMapEntry]:
