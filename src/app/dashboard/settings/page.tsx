    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <User className="h-5 w-5 text-teal" /> Profile
        </h2>
        <div className="space-y-4">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-lg font-medium text-white">{settings.name}</h3>
              <p className="text-sm text-text-muted">{settings.role}</p>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-text-muted mb-1.5 block">Full Name</label>
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-navy/50 focus-within:border-teal transition-colors">
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          {settings.darkMode ? <Moon className="h-5 w-5 text-teal" /> : <Sun className="h-5 w-5 text-teal" />} Appearance
        </h2>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-text-primary">Dark Mode</p>
            <p className="text-xs text-text-muted">Use dark theme across the application</p>
          </div>
          <button
            onClick={() => setSettings((s) => ({ ...s, darkMode: !s.darkMode }))}
            className={`w-12 h-6 rounded-full p-1 transition-colors ${settings.darkMode ? "bg-teal" : "bg-border"}`}
          >
          ...
          {[
            ...
            <div key={item.key} className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full p-1 transition-colors ${settings[item.key as keyof typeof settings] ? "bg-teal" : "bg-border"}`}
              >
                <div className={`w-4 h-4 rounded-full bg-white transition-transform ${settings[item.key as keyof typeof settings] ? "translate-x-6" : "translate-x-0"}`} />
              </button>
            </div>
          ))}
        </div>
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <Shield className="h-5 w-5 text-teal" /> Security
        </h2>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full p-1 transition-colors ${settings.twoFactor ? "bg-teal" : "bg-border"}`}
            >
              <div className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.twoFactor ? "translate-x-6" : "translate-x-0"}`} />
            </button>
          </div>
          <div>
            <label className="text-sm text-text-muted mb-1.5 block">Session Timeout (minutes)</label>
            <select
              value={settings.sessionTimeout}
              onChange={(e) => setSettings((s) => ({ ...s, sessionTimeout: e.target.value }))}
              className="px-4 py-3 rounded-xl border border-border bg-navy/50 text-text-primary text-sm outline-none focus:border-teal transition-colors w-full sm:w-48"
            >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 font-medium rounded-xl transition-all text-sm ${
            saved
              ...
              : "bg-teal text-white hover:bg-teal/80 shadow-lg shadow-teal/25"
          }`}
        >
